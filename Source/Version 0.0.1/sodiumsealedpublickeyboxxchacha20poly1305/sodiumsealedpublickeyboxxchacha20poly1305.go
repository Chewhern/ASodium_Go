package sodiumsealedpublickeyboxxchacha20poly1305
...
import (
	...
	"ASodium/sodiumsealedpublickeybox"
...
func SealedBoxCreate(
	...
	cipherText := make([]byte, len(message)+sodiumsealedpublickeybox.GetSealBytes())

	ret := C.crypto_box_curve25519xchacha20poly1305_seal(
...
func SealedBoxOpen(
	...
	if len(cipherText)-sodiumsealedpublickeybox.GetSealBytes() <= 0 {
	...
	message := make([]byte, len(cipherText)-sodiumsealedpublickeybox.GetSealBytes())

	ret := C.crypto_box_curve25519xchacha20poly1305_seal_open(
		(*C.uchar)(unsafe.Pointer(&message[0])),
		(*C.uchar)(unsafe.Pointer(&cipherText[0])),
