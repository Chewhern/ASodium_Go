package sodiumpublickeyboxxchacha20poly1305pci
...
import (
	...
	"ASodium/sodiumpublickeyboxxchacha20poly1305"
	"ASodium/sodiumrng"
...
func GetBeforeNMBytesLength() int {
	return int(C.crypto_box_curve25519xchacha20poly1305_beforenmbytes())
}
...
func CalculateSharedSecret(
	...
	if len(otherUserPublicKey) != sodiumpublickeyboxxchacha20poly1305.GetPublicKeyBytesLength() {
	...
	if len(currentUserPrivateKey) != sodiumpublickeyboxxchacha20poly1305.GetSecretKeyBytesLength() {
		return nil, errors.New("Error: Current User Private Key must be correct length")
	}

	sharedSecret := make([]byte, GetBeforeNMBytesLength())

	ret := C.crypto_box_curve25519xchacha20poly1305_beforenm(
...
func GenerateNonce() []byte {
	return sodiumrng.GetRandomBytes(sodiumpublickeyboxxchacha20poly1305.GetNonceBytesLength())
}
...
func CreateAfterNM(
	...
	if nonce == nil || len(nonce) != sodiumpublickeyboxxchacha20poly1305.GetNonceBytesLength() {
		return nil, errors.New("Error: Nonce must be correct length")
	}

	cipherText := make([]byte, len(message)+sodiumpublickeyboxxchacha20poly1305.GetMACBytesLength())

	ret := C.crypto_box_curve25519xchacha20poly1305_easy_afternm(
...
func OpenAfterNM(
	...
	if cipherText[0] == 0 {
		trim := true
		for i := 0; i < sodiumpublickeyboxxchacha20poly1305.GetMACBytesLength()-1; i++ {
			if cipherText[i] != 0 {
				trim = false
				break
			}
		...
		if trim {
			cipherText = cipherText[sodiumpublickeyboxxchacha20poly1305.GetMACBytesLength():]
		}
	}

	message := make([]byte, len(cipherText)-sodiumpublickeyboxxchacha20poly1305.GetMACBytesLength())

	ret := C.crypto_box_curve25519xchacha20poly1305_open_easy_afternm(
		(*C.uchar)(unsafe.Pointer(&message[0])),
		(*C.uchar)(unsafe.Pointer(&cipherText[0])),
...
func BoxCreateDetachedAfterNM(
	...
	mac := make([]byte, sodiumpublickeyboxxchacha20poly1305.GetMACBytesLength())

	ret := C.crypto_box_curve25519xchacha20poly1305_detached_afternm(
		(*C.uchar)(unsafe.Pointer(&cipherText[0])),
...
func BoxOpenDetachedAfterNM(
	...
	if mac == nil || len(mac) != sodiumpublickeyboxxchacha20poly1305.GetMACBytesLength() {
		return nil, errors.New("Error: MAC must be correct length")
	}

	message := make([]byte, len(cipherText))

	ret := C.crypto_box_curve25519xchacha20poly1305_open_detached_afternm(
		(*C.uchar)(unsafe.Pointer(&message[0])),
		(*C.uchar)(unsafe.Pointer(&cipherText[0])),
		(*C.uchar)(unsafe.Pointer(&mac[0])),
		C.ulonglong(len(cipherText)),
