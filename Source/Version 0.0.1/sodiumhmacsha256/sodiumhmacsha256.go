...
func GenerateKey() []byte {
	...
	C.crypto_auth_hmacsha256_keygen((*C.uchar)(&key[0]))
	return key
}
...
func GenerateKeyIntPtr() unsafe.Pointer {
	...
	if isZero == false && count < tryAttempts {
		C.crypto_auth_hmacsha256_keygen((*C.uchar)(key))
...
func VerifyMAC(mac, message, key []byte, clearKey bool) (bool, error) {
	...
	ret := C.crypto_auth_hmacsha256_verify(
		(*C.uchar)(&mac[0]),
		(*C.uchar)(&message[0]),
		C.ulonglong(len(message)),
		(*C.uchar)(&key[0]),
