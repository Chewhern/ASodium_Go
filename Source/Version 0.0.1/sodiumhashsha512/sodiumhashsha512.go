...
func GetComputedHashLength() int {
	return int(C.crypto_hash_sha512_bytes())
}

func GetStateBytesLength() int {
	return int(C.crypto_hash_sha512_statebytes())
}
...
func InitializeState() ([]byte, error) {
	...
	rc := C.crypto_hash_sha512_init(
		(*C.crypto_hash_sha512_state)(unsafe.Pointer(&state[0])),
	)
...
func UpdateState(state []byte, message []byte) ([]byte, error) {
	...
	rc := C.crypto_hash_sha512_update(
		(*C.crypto_hash_sha512_state)(unsafe.Pointer(&state[0])),
		(*C.uchar)(unsafe.Pointer(&message[0])),
		C.ulonglong(len(message)),
	)
