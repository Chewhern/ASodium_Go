...
func GetKeyBytesLength() int {
	return int(C.crypto_kdf_hkdf_sha512_keybytes())
}

func GetMinDerivedKeyBytesLength() int {
	return int(C.crypto_kdf_hkdf_sha512_bytes_min())
}

func GetMaxDerivedKeyBytesLength() int {
	return int(C.crypto_kdf_hkdf_sha512_bytes_max())
}

func GetStateBytesLength() int {
	return int(C.crypto_kdf_hkdf_sha512_statebytes())
}
...
func GenerateKey() []byte {
	...
	C.crypto_kdf_hkdf_sha512_keygen(
...
func GenerateKeyPtr() unsafe.Pointer {
	...
	if isZero == false {
		...
		C.crypto_kdf_hkdf_sha512_keygen((*C.uchar)(ptr))
...
func ExpandWithContextBytes(
	...
	C.crypto_kdf_hkdf_sha512_expand(
...
func StateInitialization(salt []byte) ([]byte, error) {
	...
	if salt == nil {
		if C.crypto_kdf_hkdf_sha512_extract_init(
			(*C.crypto_kdf_hkdf_sha512_state)(unsafe.Pointer(&state[0])),
			nil,
			0,
		) != 0 {
...
func StateUpdate(state []byte, ikm []byte, clearKey bool) ([]byte, error) {
	...
	if C.crypto_kdf_hkdf_sha512_extract_update(
		(*C.crypto_kdf_hkdf_sha512_state)(unsafe.Pointer(&state[0])),
...
func ExtractMasterKeyFromFinalState(state []byte, clearState bool) ([]byte, error) {
	...
	if C.crypto_kdf_hkdf_sha512_extract_final(
		(*C.crypto_kdf_hkdf_sha512_state)(unsafe.Pointer(&state[0])),
		(*C.uchar)(unsafe.Pointer(&masterKey[0])),
	) != 0 {
...
func StateInitializationPtr(salt []byte) unsafe.Pointer {
	...
	if salt == nil {
		C.crypto_kdf_hkdf_sha512_extract_init(
			(*C.crypto_kdf_hkdf_sha512_state)(state),
			nil,
			0,
		)
	} else {
...
func StateUpdatePtr(state unsafe.Pointer, ikm unsafe.Pointer, ikmLen int, clearKey bool) unsafe.Pointer {
	...
	C.crypto_kdf_hkdf_sha512_extract_update(
		(*C.crypto_kdf_hkdf_sha512_state)(state),
		(*C.uchar)(ikm),
		C.size_t(ikmLen),
	)
...
func ExtractMasterKeyFromFinalStatePtr(state unsafe.Pointer, clearState bool) unsafe.Pointer {
	...
	C.crypto_kdf_hkdf_sha512_extract_final(
		(*C.crypto_kdf_hkdf_sha512_state)(state),
		(*C.uchar)(masterKey),
	)
