package sodiumsecretaeadchacha20poly1305ietf
...
func ChaCha20Poly1305IETFKeyBytes() int {
	return int(C.crypto_aead_chacha20poly1305_ietf_keybytes())
}

func ChaCha20Poly1305IETFNpubBytes() int {
	return int(C.crypto_aead_chacha20poly1305_ietf_npubbytes())
}

func ChaCha20Poly1305IETFNsecBytes() int {
	return int(C.crypto_aead_chacha20poly1305_ietf_nsecbytes())
}

func ChaCha20Poly1305IETFABytes() int {
	return int(C.crypto_aead_chacha20poly1305_ietf_abytes())
}

func ChaCha20Poly1305IETFMessageBytesMax() uint64 {
	return uint64(C.crypto_aead_chacha20poly1305_ietf_messagebytes_max())
}

func ChaCha20Poly1305IETFGeneratePublicNonce() []byte {
	return sodiumrng.GetRandomBytes(ChaCha20Poly1305IETFNpubBytes())
}

func ChaCha20Poly1305IETFGenerateSecurityNonce() []byte {
	return sodiumrng.GetRandomBytes(ChaCha20Poly1305IETFNsecBytes())
}

func ChaCha20Poly1305IETFGenerateKey() []byte {
	key := make([]byte, ChaCha20Poly1305IETFKeyBytes())
	C.crypto_aead_chacha20poly1305_ietf_keygen((*C.uchar)(unsafe.Pointer(&key[0])))
	return key
}

func ChaCha20Poly1305IETFGenerateKeyPtr() unsafe.Pointer {
	key, localZero := sodiumguardedheapallocation.SodiumMalloc(ChaCha20Poly1305IETFKeyBytes())

	if localZero {
		return nil
	}

	C.crypto_aead_chacha20poly1305_ietf_keygen((*C.uchar)(key))
	sodiumguardedheapallocation.SodiumMProtectNoAccess(key)
	return key
}

func ChaCha20Poly1305IETFEncrypt(
	message []byte,
	noncePublic []byte,
	key []byte,
	additionalData []byte,
	...
	if key == nil || len(key) != ChaCha20Poly1305IETFKeyBytes() {
		return nil, errors.New("key length invalid")
	}
	if noncePublic == nil || len(noncePublic) != ChaCha20Poly1305IETFNpubBytes() {
		return nil, errors.New("public nonce length invalid")
	}
	if nonceSecurity != nil && len(nonceSecurity) != ChaCha20Poly1305IETFNsecBytes() {
		return nil, errors.New("security nonce length invalid")
	}

	cipher := make([]byte, len(message)+ChaCha20Poly1305IETFABytes())
	...
	ret := C.crypto_aead_chacha20poly1305_ietf_encrypt(
	...
	if ret != 0 {
		return nil, errors.New("ChaCha20Poly1305IETF encryption failed")
	}

	return cipher, nil
}

func ChaCha20Poly1305IETFEncryptPtr(
	message []byte,
	noncePublic []byte,
	key unsafe.Pointer,
	additionalData []byte,
...
func ChaCha20Poly1305IETFDecrypt(
	cipher []byte,
	noncePublic []byte,
	key []byte,
	additionalData []byte,
	...
	message := make([]byte, len(cipher)-ChaCha20Poly1305IETFABytes())
	var adLen C.ulonglong
	...
	ret := C.crypto_aead_chacha20poly1305_ietf_decrypt(
		(*C.uchar)(unsafe.Pointer(&message[0])),
		nil,
		ptrOrNil(nonceSecurity),
		(*C.uchar)(unsafe.Pointer(&cipher[0])),
	...
	if ret != 0 {
		return nil, errors.New("ChaCha20Poly1305IETF authentication failed")
	}
...
func ChaCha20Poly1305IETFDecryptPtr(
	cipherText []byte,
	noncePublic []byte,
	key unsafe.Pointer,
	additionalData []byte,
	...
	message := make([]byte, len(cipherText)-ChaCha20Poly1305IETFABytes())
...
func ChaCha20Poly1305IETFCreateDetached(
	message []byte,
	noncePublic []byte,
	key []byte,
	nonceSecurity []byte,
	...
	mac := make([]byte, ChaCha20Poly1305IETFABytes())
	...
	ret := C.crypto_aead_chacha20poly1305_ietf_encrypt_detached(
		(*C.uchar)(unsafe.Pointer(&cipherText[0])),
...
func ChaCha20Poly1305IETFCreateDetachedPtr(
	message []byte,
	noncePublic []byte,
	key unsafe.Pointer,
	nonceSecurity unsafe.Pointer,
...
func OpenDetachedBoxRaw(
	...
	if mac == nil || len(mac) != ChaCha20Poly1305IETFABytes() {
	...
	ret := C.crypto_aead_chacha20poly1305_ietf_decrypt_detached(
		(*C.uchar)(unsafe.Pointer(&message[0])),
		(*C.uchar)(unsafe.Pointer(&nonceSecurity[0])),
		(*C.uchar)(unsafe.Pointer(&cipherText[0])),
		C.ulonglong(len(cipherText)),
